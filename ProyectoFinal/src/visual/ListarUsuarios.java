...
public class ListarUsuarios extends JDialog {
	...
	public ListarUsuarios() {
		...
		cmbtipo.setModel(
				new DefaultComboBoxModel(new String[] { "<Todos>", "Administrador", "Basico" }));
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (index >= 0) {
					...
					miusuario = Organizadora.getInstance().getUsuario(table.getValueAt(index, 0).toString());
				}
					...
					public void actionPerformed(ActionEvent e) {
						if (miusuario != null) {
							...
							if (option == JOptionPane.OK_OPTION) {
								Organizadora.getInstance().EliminarUser(miusuario);
	...
	private void ListarUsuariosPorTipo() {
		...
		ArrayList<User> users = Organizadora.getInstance().getUsuariosPorTipo(tipoSeleccionado);
